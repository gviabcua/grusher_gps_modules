...
while (true) {
    ...
    if ($conn) {
        ...
        while (($line = fgets($conn)) !== false) {
            ...
            // Розбір повідомлення Autofon 7 (більше полів, наприклад battery, gsm signal)
            $parts = explode(',', $line);
            if (count($parts) >= 12) {
                $imei = $parts[0];
                ...
                clilogTracker("IMEI: $imei | $datetime->format('Y-m-d H:i:s') | Lat: $latitude, Lon: $longitude, Speed: $speed, Battery: $battery, GSM: $gsmSignal", $protocol_name);
